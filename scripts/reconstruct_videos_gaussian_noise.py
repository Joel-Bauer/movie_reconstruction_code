...
# get gaussian noise stimulus
gaus_noise_vids = np.load('utils_reconstruction/gaussian_noise_movies.npz')
vids = gaus_noise_vids['video']
## format vids
vids = ((vids-vids.flatten().mean())/vids.flatten().std())*255/5 + 255/2
vids = np.pad(vids, ((0,0),(11,17),(0,0),(0,0),(0,0),(0,0)), 'constant', constant_values=125) # center stim on mask positions (hard coded for now)
vids = np.transpose(vids, (3,4,5,2,0,1))
# contrast normalize each video
for rep in range(0,vids.shape[0]):
    for s in range(0,vids.shape[1]):
        for t in range(0,vids.shape[2]):
            vids[rep,s,t] = ((vids[rep,s,t]-np.nanmean(vids[rep,s,t].flatten()))/np.nanstd(vids[rep,s,t].flatten()))*255/5 + 255/2
        ...
        for mouse_index in animals:
            for temporal_length_constant in range(vids.shape[2]-1,-1,-1): #first slow then fast
                for spatial_length_constant in range(0,vids.shape[1]): # first small then big
                    print(f'model fold {model_list[current_model]} mouse {mouse_index} rep {rep} spatial {spatial_length_constant} temporal {temporal_length_constant}')
                    ...
                    fig.savefig(save_path+f'reconstruction_summary_m{mouse_index}_r{rep}s{spatial_length_constant}t{temporal_length_constant}.png')
                    ...
                    # save dictionary
                    np.save(save_path+f'reconstruction_summary_m{mouse_index}_r{rep}s{spatial_length_constant}t{temporal_length_constant}.npy',recon_dict)   
